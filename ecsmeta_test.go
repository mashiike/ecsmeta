...
func TestNew(t *testing.T) {
	...
	withLogger := ecsmeta.WithLogger(log.New(os.Stderr, "", log.LstdFlags))
	cases := []struct {
		name     string
		opts     []ecsmeta.Option
		...
		{
			name: "v3",
			opts: []ecsmeta.Option{withLogger},
			envs: map[string]string{
				"ECS_CONTAINER_METADATA_URI": server.URL + "/v3",
			},
			query:    "[.Containers[].Image]",
			expected: `["amazon/amazon-ecs-pause:0.1.0","nrdlngr/nginx-curl"]`,
		},
		{
			name: "v4",
			opts: []ecsmeta.Option{withLogger},
			envs: map[string]string{
				"ECS_CONTAINER_METADATA_URI": server.URL + "/v4",
			},
			query:    "[.Containers[].Image]",
			expected: `["mreferre/eksutils"]`,
		},
		{
			name:     "none",
			opts:     []ecsmeta.Option{withLogger},
		...
		},
		{
			...
			envs: map[string]string{
				"ECS_CONTAINER_METADATA_URI": server.URL + "/invalid",
			},
			query:    "[.Containers[].Image]",
			expected: `null`,
		},
	...
	for _, c := range cases {
		...
		t.Run(c.name, func(t *testing.T) {
			data := ecsmeta.New(c.opts...)
			r, _ := data.Query(c.query)
			if r.String() != c.expected {
				t.Errorf("unexpected query result. actual:%s", r.String())
			}
		})
	}
}
